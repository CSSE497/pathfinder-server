...
import play.api.libs.json.{JsNumber, JsObject, JsNull, Json}

class RouteTest extends PlaySpec {

    val routeJson = Json.parse("""{
                "vehicle":{"id":9,"latitude":2,"longitude":3,"status":"Online","metadata":{"capacity":1}},
                "actions":[
    ...
    val coms = Seq(
        Commodity(1,2,3,4,5,CommodityStatus.Waiting,JsObject(Seq("param" -> JsNumber(6)))),
        Commodity(7,8,9,10,11,CommodityStatus.Waiting,JsObject(Seq("param" -> JsNumber(12)))),
        Commodity(13,14,15,16,17,CommodityStatus.Waiting,JsObject(Seq("param" -> JsNumber(18))))
    )
    ...
    val vehicle = Vehicle(9, 2, 3, VehicleStatus.Online,JsObject(Seq("capacity" -> JsNumber(1))))
